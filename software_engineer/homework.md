...
以你所在部门为目标组织、描述这个组织的业务状况，指出外部涉众和内部涉众。给出主要的三类业务用例和对应的业务执行者，画出业务用例图，并对一个核心业务用例活动图描述业务流程。 
...
对描述了业务流程的那个核心业务用例进行分析，给出与业务分析模型相关的顺序图（要求能把内部业务工作者和业务实体展现出来）和实体类图。
...
根据业务建模作业中对应的业务系统，探索那些地方可以流程自动化的，给出系统的功能性需求特性列表；并找出所有的系统执行者和系统用例。

# homework5 用例详细描述作业

从你系统用例图里选2个用例，详细描述用例，写出用例规约，并画出用例场景对应的活动图

# homework6 分析建模静态模型

根据需求的用例模型，分析系统的概念类，给出概念类的主要属性，和所有类之间的关系，并画出类图

# homework7 点餐用例的分析建模

描述点餐用例的系统分析的静态模型和动态模型
